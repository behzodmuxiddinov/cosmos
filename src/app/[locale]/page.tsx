...
export default  function Home() {
  ...
  const { mode, products, setProducts } = useGlobalContext()
  ...
  return (
    <main className={`w-full min-h-screen flex flex-col items-center py-5 ${mode ? 'bg-white' : 'bg-darkblue text-white'}`}>
      ...
      <div className='w-[90%] md:[95%] hidden md:flex md:justify-start sm:justify-center my-5'>
        <select name="select" id="selectfilter" onChange={changeFilter} className='text-black p-1 outline-none font-semibold text-lg w-1/3 rounded-sm'>
          {
            filterBtns.map(item => (
      ...
      <div className="grid 3xl:grid-cols-4 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 sm:grid-cols-1 gap-x-10 gap-y-6 justify-items-center py-9">
        {
          products.filter(item => {
            if(filter){
              return item.category.toLowerCase().includes(filter.toLowerCase()) || item.name.toLowerCase().includes(filter.toLowerCase()) || item.comp.toLowerCase().includes(filter.toLowerCase())
            }
            return item
          })
          .filter(item => {
            switch(filterby){
              case 'all':
                return item
              case `${filterby}`:
                return item.category.toLowerCase().includes(filterby) || item.name.toLowerCase().includes(filterby) || item.comp.toLowerCase().includes(filterby)
            }
          })
          .map(product => {
